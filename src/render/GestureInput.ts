...
export function composeGestureFrame(hands: TrackedHand[], face: GestureFrame['face'], pointer: TrackedHand | null, mirrored: boolean, _demo = false): GestureFrame {
  ...
  const indexSpeed = primary ? speed(primary.indexVelocity) : 0;
  ...
  else if (primary?.pinchStrength && primary.pinchStrength > .6 && indexSpeed < .06) activeGesture = 'charge';
  ...
  const charge = primary && indexSpeed < .08 ? Math.min(1, (primary.pinchStrength + .35)) : 0;

  return { hands: source, face, pointer, activeGesture, swipeVelocity, twoHandDistance, charge, demo: false, mirrored, suggestedMode: null };
}
