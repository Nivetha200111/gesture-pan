...
export function usePointerFallback(target: React.RefObject<HTMLElement | null>) {
  ...
  useEffect(() => { const el = target.current; if (!el) return;
    ...
    const move = (e: PointerEvent) => { const p = norm(e); const drag = e.buttons ? .75 : .2; last.current.pinch = e.shiftKey || e.button === 2 ? .85 : Math.max(0, last.current.pinch * .96); setHand(makeHand(p.x, p.y, last.current.x, last.current.y, Math.max(last.current.pinch, drag * .25))); last.current.x = p.x; last.current.y = p.y; last.current.t = performance.now(); };
